...
ロボ側は、今のペアIDで最初に受け取ったコントローラーのMACアドレスを覚えます。その後は同じペアIDでも、別のコントローラーから来た入力は無視します。ロボ側のG39でペアIDを変えると、このコントローラーロックも解除されます。USBシリアルで `U` を送っても解除できます。
...
ロボ側とコントローラー側の両方を同じIDにしてください。番号が違う場合、ロボ側はそのパケットを無視します。
